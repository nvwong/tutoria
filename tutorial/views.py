...
def cancel(request):
    try:
        ...
        if (selected_session.start_time - timezone.now() <= datetime.timedelta(hours=24)):
            return render(request, 'session_list.html', {'error_message': "Cannot cancel sessions in coming 24 hours.",})
        else:
